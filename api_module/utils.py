...
def connect_mongodb() -> MongoClient:
    ...
    try:
        mongo_client = MongoClient('mongodb://127.0.0.1:27017') # IP LOCAL
        mongo_client.server_info()  # Vérifier la connexion
        print("Connecté à MongoDB en utilisant localhost (127.0.0.1)")
        return mongo_client
    ...
    try:
        ...
        mongo_client = MongoClient(f'mongodb://root:example@{ssh_ip}:27017')
        mongo_client.server_info()  # Vérifier la connexion
        print(f"Connecté à MongoDB en utilisant l'adresse IP de la connexion SSH : {ssh_ip}")
        return mongo_client
    except Exception as e2:
        print(f"Erreur lors de la connexion à MongoDB en utilisant l'adresse IP de la connexion SSH : {e2}")
